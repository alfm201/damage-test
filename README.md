...
`S` = 소환 능력치 적용 배율(%)<br>
`SC` = 소환 공격력(속성력) 계수<br>
대부분의 소환 공격은 2,000을 사용하지만, 일부 소환체·공격 동작은 다른 값을 사용한다.
* SC는 직타 스킬 툴팁의 `스킬 계수`와 동일한 척도로 표현한 값이다.
* 실제 소환체의 공격력·속성력에 곱할 때는 직타와 동일하게 **(SC + 100) / 50**으로 변환한다.<br>
예를 들어 SC=2000이면 적용 배수는 42이다.
* 캐릭터 능력치를 계승하는 소환 배율(%)과는 별개이다.
